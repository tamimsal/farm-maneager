...
function readWsUrl(initialUrl?: string): string {
  ...
  // PRODUCTION: Default to HTTPS WebSocket URL
...
export function useFarmSocket(url?: string): FarmSocketState & { sendPumpCommand: (command: 'start' | 'stop', duration?: number) => void } {
  ...
  useEffect(() => {
    ...
    console.log('[FarmSocket] Production mode: HTTPS WebSocket connection');
  }, [wsUrl]);
  ...
  useEffect(() => {
    function connect() {
      try {
        ...
        ws.onopen = () => {
          setIsConnected(true);
          console.log('[FarmSocket] Connected to production WebSocket');
        };
        ...
        ws.onclose = () => {
          ...
          console.log('[FarmSocket] WebSocket disconnected, attempting reconnection...');
